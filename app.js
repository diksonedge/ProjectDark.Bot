...
vk.setOptions({ token: 'dc0075dc3a56004231182613871f2189af7d4d2b276cf4654122fbc6f0b37cf4aef1db5c3481f6e7197d2', pollingGroupId: 171547488 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club171547488\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club171547488\|(.*)\]/ig, '').trim();
